...
$tty = Expect->spawn(qq{timeout -k 60 55 make qemu});
$tty->expect($TO, "init: starting sh");
sleep(1);

$tty->send("test2\n");
$tty->expect($TO, "test2 done");
check_output($tty, "test2", "stdin", [0, 6, 20, 0]);
check_output($tty, "test2", "stdout", [1, 6, 20, 0]);
check_output($tty, "test2", "stderr", [2, 6, 20, 0]);
...
check_output($tty, "test3", "hello1", [2, 6, 26, 0]);
